...
def run():
    ...
    for index, name in enumerate(names):
        ...
        frame_path = f"./run/sims/{name}/frames"
        ...
        while iterator.isRunning():
            ...
            for texture in [1, 0]:
                ...
                for posID, position in enumerate([positions[4]]):
            ...
            stitched.paste(r_imgs[0], (0,0, half_w, half_h))
            stitched.paste(l_imgs[0], (half_w,0, width, half_h))
            stitched.paste(r_imgs[1], (0, half_h,half_w, height))
            stitched.paste(l_imgs[1], (half_w, half_h, width, height))
            draw = ImageDraw.Draw(stitched)
            font = ImageFont.load_default()
            draw.line([(half_w, 0), (half_w, height)], fill="white", width = 2)
            draw.line([(0, half_h), (width, half_h)], fill="white", width = 2)

            stitched.resize((1920, 1080)).save(f"{frame_path}/{frame}.png")
            del(l_imgs)
            del(r_imgs)
            frame += 1
            frames.append(stitched)
            iterator.increment()
        frames[0].save(f"{frame_path}/simulation.gif", save_all = True, append_images=frames[1:], duration = 500)
